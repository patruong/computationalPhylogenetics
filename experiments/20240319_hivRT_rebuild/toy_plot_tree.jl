...
# Merge subtrees under a new root "A"
nwk = "(A(B,C),D);"
nwk = "(A{1}:1(B{1}:1,C{2}:1),D{2}:1);"

nwk = "(A(B,C),D(E,F));"
nwk = "(A{1}:1(B{1}:1,C{1}:1),D{2}:1(E{2}:1,F{2}:1));"


nwk = "($treated_nwk_part, $naive_nwk_part)"

## Output the merged Newick string
#println(merged_nwk)
#nwk = "($(naive_nwk):0.01, $(treated_nwk))"
