...
parser = ArgParse.ArgumentParser(prog="time_parvoVP_dataset.jl", description="Benchmark difFUBAR grid search with different pruning strategies")
data_dir_arg = ArgParse.add_argument(parser, "--data-dir", type=String, required=true, help="Directory to read input data")
output_dir_arg = ArgParse.add_argument(parser, "--output-dir", type=String, required=true, help="Directory to save output files")
ArgParse.parse(parser)

file_dir = parsed_args["data_dir"]
